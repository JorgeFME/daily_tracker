...
def _render_resumen(ws, proyecto_nombre, generado_en, actividades, evidencias_por_actividad, export_context=None):
    ...
    ws.title = "Resumen"
    ws.sheet_view.showGridLines = True  # Mantener cuadrícula sutil ayuda a la lectura corporativa
    ...
    for offset, prioridad in enumerate(("Alta", "Media", "Baja"), start=0):
        ...
        color = {"Alta": C_ALTA_T, "Media": C_MEDIA_T, "Baja": C_BAJA_T}.get(prioridad, C_DARK)
...
def _render_actividades(ws, proyecto_nombre, generado_en, actividades, evidencias_por_actividad, export_context=None):
    ...
    for column_index, (header, width) in enumerate(headers, start=1):
        ...
        cell.fill = _fill(C_DARK)  # Encabezados de tabla robustos e institucionales
        cell.alignment = C_ALN
        cell.border = BORDER
    ...
    for row_index, actividad in enumerate(actividades, start=4):
        ...
        row_bg = C_WHITE if row_index % 2 == 0 else C_GRAY
        ...
        row_data = [
            ...
            _safe(actividad.get("NOMBRE_ENTREGABLE")),     
            tipo_actividad,                                
        ...
        ws.row_dimensions[row_index].height = max(std_row_height, 15 * max(len(desglose_lineas), 1))
        ...
        for column_index, value in enumerate(row_data, start=1):
            ...
            if column_index in (5, 6, 7): 
                ...
                color = {"Alta": C_ALTA_T, "Media": C_MEDIA_T, "Baja": C_BAJA_T}.get(prioridad_texto, C_DARK)
                cell.fill = _fill(bg)
                ...
                cell.font = _font(bold=True, color=ESTATUS_TXT.get(estatus_norm, C_DARK))
        ...
        if horas_desarrollo > 40:
            ...
            horas_cell.fill = _fill(C_ALTA)
            horas_cell.font = _font(bold=True, color=C_ALTA_T)
    ...
    total_caption = ws.cell(row=total_row, column=1, value="TOTAL HORAS DE DESARROLLO   ")
    ...
    total_hours_cell.font = _font(bold=True, color=C_WHITE, size=10)
